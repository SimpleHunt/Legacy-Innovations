...
export default function SalesReportPage() {
  ...
  const handleFilter = async () => {
    ...
    const res = await fetch(
      `/api/reports/sales?startDate=${startDate}&endDate=${endDate}&page=${page}`
    );
  ...
  const downloadExcel = () => {
    window.open(
      `/api/reports/sales/excel?startDate=${startDate}&endDate=${endDate}`,
      "_blank"
    );
  };
  ...
  const downloadPDF = () => {
    window.open(
      `/api/reports/sales/pdf?startDate=${startDate}&endDate=${endDate}`,
      "_blank"
    );
  };
  ...
  return (
    <div className="bg-white p-4 rounded-md flex-1 m-4 mt-0">
      ...
      <div className="flex items-center justify-between">
        <h1 className="hidden md:block text-lg font-semibold">Sales Report</h1>
